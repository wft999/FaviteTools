odoo.define('padtool.cfields', function (require) {
"use strict";
...
var FieldArrayNumeric = basic_fields.DebouncedField.extend({
    tagName: 'span',
    supportedFieldTypes: ['arraynumeric'],
     ...
     */
    _getValue: function () {
        return _.map(this.inputs,$input=>$input.val());
    },
    ...
    _prepareInput: function ($input) {
    	...
    	for(var i = 0;i<this.value.length;i++){
    		this.inputs[i] = $("<input/>")
    		this.inputs[i].addClass('o_input o_input_double o_field_integer o_field_number');
            this.inputs[i].attr({
...
field_registry.add('arraynumeric', FieldArrayNumeric);
